...
def test__encrypted_node_read_accessors_use_decrypted_shadow_subtree():
    """Verify unlocked encrypted nodes expose decrypted content via reads."""
    ...
    assert node.plaintext == "this is an attributed node with HTML content"
    assert node.attributes == {"type": "test"}
